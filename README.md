...
- **Story Mode (campaign) + main-menu rename** — the main menu's first button is now **"FREE ROAM"** (the open world, formerly "Start Game") and the second is **"STORY MODE"** (formerly the disabled "Load Game"), which opens a new campaign screen (`story_mode`). It shows the **prologue** (Politécnico outbreak: at the ENCB, Prankedy accidentally creates a corrosive, latex-piercing substance) and a **school picker** — only **ESCOM** is playable for now; **FES Aragón** and **UAM** appear but are disabled (in development). "LOAD GAME" inside is disabled (no save system yet). Picking a school + "START" sets the spawn via `WorldMapViewModel.setStorySpawn(...)` and enters the world. School data in `domain/models/SchoolCatalog.kt`.
