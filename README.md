...
- **FES Aragón interior (its own Interiors room)** — the **"Entrada FES Aragón"** door enters the **Interiors engine** (full controls, options, combat, ZONA/health/MODO HUD) but in **its own room**, not the ESCOM lobby. The route is parametrized `interiores_zombies?startRoom={id}`; the FES door passes `startRoom=fes_interior` (`ZombieRoomCatalog.FES_ID`, FES background, safe LOBBY type, exit-to-world). The Interiors server (`MultiplayerInteriores/server.js`) knows the room. The FES door is also moved next to the FES teleport point.
...
- **Interior de FES Aragón (su propia sala de Interiores)** — la puerta **"Entrada FES Aragón"** entra al **motor de Interiores** (controles, opciones, combate y HUD ZONA/vida/MODO) pero en **su propia sala**, no en el lobby de ESCOM. La ruta es parametrizable `interiores_zombies?startRoom={id}`; la puerta FES pasa `startRoom=fes_interior` (`ZombieRoomCatalog.FES_ID`, fondo FES, tipo LOBBY seguro, salida al mapa). El servidor de Interiores (`MultiplayerInteriores/server.js`) conoce la sala. Además la puerta FES se mueve junto al teletransporte de FES.
