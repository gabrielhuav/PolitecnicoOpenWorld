...
The latest feature adds a new side mission and the underlying architecture to support more in the future:

- **Side Mission: "Carrera del Politécnico"** — a new opt-in side mission activated by talking to a special NPC ("Entrenador Politécnico") positioned near the ESCOM main entrance. Accepting launches a 3-2-1 countdown briefing screen (`race_mission` route, `RaceMissionScreen`/`RaceMissionViewModel`) and then starts a 60-second race HUD on the world map. The player must reach the finish-line marker at the far end of the Zacatenco campus before time runs out. Hitting a pedestrian NPC during the race adds a 5-second penalty. Winning for the first time awards a unique collectible ("Trofeo de la Carrera del Politécnico"); subsequent completions show the achieved time only. Both victory and timeout show dedicated overlays with retry support. The mission can be attempted indefinitely. Architecture: `domain/models/SideMission.kt` + `EntrenadorLocation.kt` (domain); `features/side_mission_race/` (ViewModel + Screen); `WorldMapRaceMission.kt` (WorldMapViewModel extension); new fields in `WorldMapState`; `CollectibleDao.insertCollectible`; `CollectibleRepository.ensureRaceCollectible`; `SettingsRepository` race-reward flag.

The previous integration work centers on the **multiplayer back end**, which now spans two servers, plus the previously bundled feature/performance PRs:
...
La última feature agrega una misión secundaria y la arquitectura base para soportar más en el futuro:

- **Misión Secundaria: "Carrera del Politécnico"** — nueva misión opcional que se activa hablando con el NPC especial "Entrenador Politécnico" situado cerca de la entrada principal de ESCOM. Al aceptar, aparece una pantalla de briefing con cuenta regresiva 3-2-1 (ruta `race_mission`, `RaceMissionScreen`/`RaceMissionViewModel`) y luego arranca un HUD de 60 segundos en el mapa. El jugador debe llegar al marcador de meta en el extremo norte del campus antes de que se acabe el tiempo. Golpear un NPC peatón durante la carrera añade 5 segundos de penalización. Ganar por primera vez otorga un coleccionable exclusivo ("Trofeo de la Carrera del Politécnico"); los intentos siguientes solo muestran el tiempo logrado. Tanto la victoria como el tiempo agotado muestran overlays con opción de reintentar. La misión es infinitamente reintentable. Arquitectura: `domain/models/SideMission.kt` + `EntrenadorLocation.kt` (dominio); `features/side_mission_race/` (ViewModel + Screen); `WorldMapRaceMission.kt` (extensión de WorldMapViewModel); nuevos campos en `WorldMapState`; `CollectibleDao.insertCollectible`; `CollectibleRepository.ensureRaceCollectible`; flag de recompensa en `SettingsRepository`.

El trabajo de integración anterior se centraba en el **back end multijugador**, que ahora abarca dos servidores, además de los PRs de funcionalidad/rendimiento previos:
