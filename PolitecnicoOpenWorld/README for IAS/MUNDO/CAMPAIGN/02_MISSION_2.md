...
|---|---|---|---|
| 1 | `PHASE_HIDE` | `m2_esconderse_policia` | 🆕 (2026-07-08) DENTRO del **lobby de la ESCOM**: 4 policías `m2cop_*` (skin `POLICIA_CDMX`) entran por la puerta y patrullan la sala (cada ~7 s UNO barre hacia ti). Si te tienen a <120 px por >2.5 s → te reconocen → **MISIÓN FALLIDA** (sales al mapa, REINTENTAR). Aguanta **35 s** (countdown en HUD) → se rinden, corren a la puerta y se van → cumplido (`completeMission2Hide` avanza a fase 2). 🆕 (2026-07-13) **Tope de retirada** `HIDE_EVAC_TIMEOUT_MS=10 s`: si un policía se atora contra una colisión (autos del lobby) camino a la puerta, pasado el tope los rezagados desaparecen y la fase CUMPLE igual (antes quedaba sin completar para siempre). HUD menos intrusivo: el widget de objetivo se PLIEGA a solo el título tras ~6 s (`ObjectivesWidget(compact=true)`) y el countdown es más chico/translúcido. Armado runtime vía `setMission2Hide`; ver 05/09. |
...
| 5 | `PHASE_BACKPACK` | `m2_recuperar_mochila` | 🆕 (2026-07-13) Se entra por el **flujo NORMAL de interiores**: puerta ESCOM → lobby → **Edificio Principal** (`za_edificio`, puerta "Salón 2009") → salón `escom_salon_m2` (ya NO hay redirect en la puerta exterior; el TP dev sí va directo). La **LATA APESTOSA es un ÍTEM del inventario** (Prankedy te la da al cerrar la plática — `grantMission2StinkCan`, asset `CAMPAIGN/MISSION2/lata_apestosa.png`, slot 2): X en el salón EN CLASES la lanza (la CONSUME; sin lata el salón es un aula normal) → los alumnos EVACÚAN (`evacuateAmbientNpcs`) → aparece la **mochila** (`CAMPAIGN/MISSION2/mochila_prankedy.png`) → X la recoge → `onMission2BackpackRecovered` → `completeMission2Backpack()` → `PHASE_DONE`. La puerta del salón regresa al edificio. |
...
`HIDE_SWEEP_EVERY_MS=7000`, `HIDE_EVAC_TIMEOUT_MS=10000` (🆕 tope de la retirada).
...
(`inventoryUnlockedSlots`; 🆕 2026-07-13 el default es **2 slots** — llave M1 + lata M2 conviven),
y las misiones 2/3 se SIGUEN desde el **registro de misiones** (Opciones → "Misiones"; ya no
arrancan solas — ver `00_OVERVIEW`). 🆕 (2026-07-13) La llave de la M1 y la lata son **objetos de
misión**: no se pueden desechar mientras las misiones 1-2 estén en curso (`missionItemsLocked`;
la lata nunca se desecha — se consume al lanzarla).
