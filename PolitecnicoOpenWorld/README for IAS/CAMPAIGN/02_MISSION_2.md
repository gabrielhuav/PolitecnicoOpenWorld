...
| 5 | `PHASE_BACKPACK` | `m2_recuperar_mochila` | La puerta de la ESCOM REDIRIGE al salón `escom_salon_m2` (`WorldMapInteractions`, solo en esta fase y solo la ruta default de ESCOM). Salón EN CLASES (NPCs ambientales IPN/docente). X = lanzar **LATA APESTOSA** → los alumnos EVACÚAN corriendo a la puerta (`evacuateAmbientNpcs`) → aparece la **mochila 🎒** (emoji, sin asset) → X la recoge → `onMission2BackpackRecovered` → `completeMission2Backpack()` → `PHASE_DONE`. |
