...
- **🆕 EDITOR del Debug Interiores (`WorldMapDebugEditor.kt` + `InteriorDebugEditorPanel.kt`):** con el overlay activo aparece un panel para **EDITAR** las líneas caminando con el jugador (mismo patrón que el "Creador de Rutas"): eliges herramienta `DebugEditTool` (WALL=barda roja, BLOCK=zona roja, NAV_PED=verde peatonal, NAV_CAR=naranja autos), **"Capturar"** suelta un punto en tu posición, **"Terminar"** commitea la forma, **Deshacer/Limpiar**, y **Exportar/Importar JSON** (formato `exterior_collisions.json` + sección `navPaths`). Estado nuevo en `WorldMapState`: `debugEditTool`, `debugEditPoints`, `debugEditWalls`, `debugEditBlocks`, `debugEditNavPed`, `debugEditNavCar`. Render en vivo: overlay AISLADO en `NativeOsmMap` (tags +700/+710/+720), sin tocar el overlay de archivo. Web/Google nativo = pendiente (igual que el resto del overlay).
