...
- **Story Mode (campaign) + main-menu rename** — the main menu's first button is now **"FREE ROAM"** (the open world, formerly "Start Game") and the second is **"STORY MODE"** (formerly the disabled "Load Game"), which opens a new campaign screen (route `story_mode`, `StoryModeScreen`/`StoryModeViewModel`). It shows the **prologue** (Politécnico outbreak: at the ENCB, Prankedy accidentally creates a corrosive, latex-piercing substance) and a **school picker** — only **ESCOM** is playable for now; **FES Aragón** and **UAM** are shown but disabled (in development). "LOAD GAME" inside it is disabled (no save system yet). Picking a school + "START" sets the campaign spawn via `WorldMapViewModel.setStorySpawn(lat, lon)` (a non-gated spawn that re-arms the world-load gates) and enters `world_map`. School data lives in `domain/models/SchoolCatalog.kt`.
