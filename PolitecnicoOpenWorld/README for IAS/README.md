...
- **FES Aragón interior (its own Interiors room)** — the **"Entrada FES Aragón"** door enters the **Interiors engine** (`ZombieGameScreen`: full movement + action buttons, options/settings/skin, combat, and the ZONA/health/MODO HUD) but starts in **its own room**, not the ESCOM lobby. The route is now parametrized: `interiores_zombies?startRoom={id}` → `ZombieGameViewModel.startRoomId`; the FES door passes `startRoom=fes_interior` (`ZombieRoomCatalog.FES_ID`, background `FES_Arg_int.webp`, **type LOBBY = safe zone, no zombies**, with an exit-to-world door). The **Interiors server** (`MultiplayerInteriores/server.js`) knows the room (`ROOMS['fes_interior']`, LOBBY → relay-only). The FES door is also **moved next to the FES teleport point** (and `WorldMapDesigner` relocates an existing far-away one). Doors route by name (robust substring): `…FES → fes_interior`, sports venues → their `interior_*`, ESCOM doors → the lobby. To give FES its own zombies/rooms later: flip it to `BUILDING` (client + server) or add `fes_*` rooms.
...
- **Interior de FES Aragón (su propia sala de Interiores)** — la puerta **"Entrada FES Aragón"** entra al **motor de Interiores** (`ZombieGameScreen`: movimiento + botones de acción, opciones/ajustes/skin, combate y el HUD ZONA/vida/MODO) pero arranca en **su propia sala**, no en el lobby de ESCOM. La ruta ahora es parametrizable: `interiores_zombies?startRoom={id}` → `ZombieGameViewModel.startRoomId`; la puerta FES pasa `startRoom=fes_interior` (`ZombieRoomCatalog.FES_ID`, fondo `FES_Arg_int.webp`, **tipo LOBBY = zona segura, sin zombis**, con puerta de salida al mapa). El **servidor de Interiores** (`MultiplayerInteriores/server.js`) conoce la sala (`ROOMS['fes_interior']`, LOBBY → solo relay). Además la puerta FES se **mueve junto al teletransporte de FES** (y `WorldMapDesigner` reubica una existente que esté lejos). Las puertas enrutan por nombre (substring): `…FES → fes_interior`, deportivos → su `interior_*`, puertas ESCOM → el lobby. Para darle zombis/cuartos propios a FES: pásala a `BUILDING` (cliente + server) o añade salas `fes_*`.
