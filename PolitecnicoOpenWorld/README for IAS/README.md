...
- **Landmark Editor Improvements** — Added a **scrollable list** to the "Add Asset" dialog in Designer Mode to accommodate more buildings. Added the **Central Norte** building asset to the catalog.
...
- **GTA-lite NPC AI (low-end optimized, host-only)** — NPCs now have a **personality trait** (`PASSIVE`/`COWARD`/`AGGRESSIVE`, weighted at spawn). You can **run over pedestrians** while driving (speed-scaled damage, smaller 0.4 hitbox, +1 wanted star; witnesses panic). **Carjacking an occupied car** makes the ousted driver react by trait: cowards flee, aggressive ones **chase and punch you**. **Aggressive NPCs hit back** for a few seconds when you melee them and they survive. **Two-way traffic** is enabled (randomized spawn direction + existing right-side lane offset). **Driving collisions** feature smooth automatic overtaking at any speed when driving straight (with a 1.5s auto-centering magnet), and Toretto-style shoving + 💥 when steering manually into cars. All checks are cull-radius-bound with early-outs and the AI fields aren't serialized. *(Car-vs-car collisions are intentionally left as future work.)*
...
**⚠️ CRITICAL CONTEXT FOR AI ASSISTANTS:** The complete and detailed project context (function signatures, state management rules, strict MVVM patterns, gotchas, network protocols, etc.) is **NOT** fully contained here to avoid overloading context windows. The **full compressed context** lives exclusively in the `PolitecnicoOpenWorld/README for IAS` directory. Before touching any code, an AI assistant **MUST** read `00_INDEX.md` and `09_CONVENTIONS_GOTCHAS.md` in that folder.
...
- **Mejoras en el Editor de Landmarks** — Se añadió una **lista con scroll** al diálogo "Agregar Asset" en el Modo Diseñador para dar cabida a más edificios. Se integró el asset del edificio **Central Norte** al catálogo.
...
- **IA de NPCs estilo GTA (optimizada para gama baja, solo en el host)** — los NPCs tienen una **personalidad** (`PASSIVE`/`COWARD`/`AGGRESSIVE`, peso aleatorio al spawn). Puedes **atropellar peatones** conduciendo (daño según la velocidad, hitbox reducida al 0.4x, +1 estrella de búsqueda; los testigos huyen). Al **robar un coche ocupado**, el conductor desalojado reacciona según su rasgo: los cobardes huyen, los agresivos **te persiguen y te golpean**. Los **NPCs agresivos contraatacan** unos segundos si los golpeas y sobreviven. **Tráfico en doble sentido** activado (dirección de spawn aleatoria + el desplazamiento de carril a la derecha ya existente). **Colisiones de manejo** afinadas con un rebase automático suave a cualquier velocidad en recta (con imán de auto-centrado de 1.5s), y choques estilo Toretto + 💥 al dar volantazos voluntarios. Todo con cortes tempranos acotados al radio de culling y sin serializar los campos de IA. *(Las colisiones coche-coche quedan como trabajo futuro a propósito.)*
