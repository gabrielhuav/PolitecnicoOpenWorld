...
| 🆕 Save/load game (full JSON state) | `data/repository/SaveGameRepository.kt` (`GameSaveData`/`SavedNpc`, `pow_campaign_save.json`) + `features/map_exterior/viewmodel/WorldMapSaveGame.kt` (`saveGame`/`loadGame`/`buildSaveData`/`restoreSaveData`); manual "Guardar partida" in Options + auto-save on exit (`MainActivity`) |
