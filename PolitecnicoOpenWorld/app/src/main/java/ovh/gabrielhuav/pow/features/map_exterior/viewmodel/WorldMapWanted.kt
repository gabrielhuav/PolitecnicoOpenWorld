...
// constantes viven en `wantedManager` (WantedManager, manager 4/6 — ver
// CHECKPOINT_SENIOR_refactor.md); aquí queda solo la orquestación que mezcla red/policía/
// Context/IO y DELEGA los writes al manager. NO duplicar estos nombres como miembros.
// ─────────────────────────────────────────────────────────────────────────────
...
// Sube el nivel de búsqueda (con tope) y reinicia el contador de impunidad. Delega en el manager;
// el guard del APOCALIPSIS se queda aquí porque necesita globalZombieMode (del estado del VM).
internal fun WorldMapViewModel.raiseWantedLevel(amount: Int = 1) {
    ...
    wantedManager.raiseWantedLevel(amount, System.currentTimeMillis())
}

// Baja el nivel de búsqueda gradualmente cuando dejas de delinquir (delega en el manager).
internal fun WorldMapViewModel.tickWantedDecay(now: Long) {
    wantedManager.tickWantedDecay(now)
}
...
// Gestiona el aviso y el descenso forzado del vehículo. La MÁQUINA de carjack (timer + aviso)
// vive en wantedManager; aquí quedan las lecturas del VM (vehicleSpeed/MAX_SPEED), el texto
// localizado y el descenso forzado (forceExitVehicle).
...
internal fun WorldMapViewModel.runPoliceTick(location: GeoPoint) {
    ...
    val wanted = wantedManager.state.value.wantedLevel
    ...
    // viejos (>450 ms), para dibujar un trazo breve desde el policía hacia ti (delega en el manager).
    val freshShots = if (tick.shots.isNotEmpty())
        tick.shots.map { PoliceShot(it.first, it.second, now) } else emptyList()
    wantedManager.mergeAndPrunePoliceShots(freshShots, now, 450L)
